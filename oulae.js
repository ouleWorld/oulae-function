...
const isFunction = s => Object.prototype.toString.call(s) === '[object Function]'
const isSet = s => Object.prototype.toString.call(s) === '[object Set]'
const isUndefined = s => Object.prototype.toString.call(s) === '[object Undefined]'
const isNull = s => Object.prototype.toString.call(s) === '[object Null]'
...
const deepCloneRecu = (obj) => {
    // 实现深拷贝函数
    if (typeof obj !== 'object') {
        return obj
    }
    let object = obj
    let result = isObject(object) ? {} : []
    let aProps = Object.getOwnPropertyNames(object)
    aProps.forEach((ele, index, array) => {
        if (typeof object[ele] === 'object') {
            result[ele] = deepCloneRecu(object[ele])
        } else {
            result[ele] = object[ele]
        }
...
/*
 *@example-cin: 2020-02-12T04:36:53.000+0000
 *@example-cout: 2020-02-12 12:36:53
*/
const dealTimeForJava = (t) => {
    if (!t) {
        return ''
    }
    let dataee = new Date(t).toJSON();
...
// 字符串操作:
// 判断一个字符串中是否含有大写字母,如果有，则返回第一个大写字母的下标，否则返回-1
const hasUppercaseLetter = (string) => {
    let index = -1
    let str = string
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        // ASCII码值：A：65 Z：90
        // 判断方式1：使用ASCII码值判断一个字符是否是大写
        // let judge = temp > 64 && temp < 91
        // 补充: 判断一个字符是否是小写
        // let judge = temp > 96 && temp < 122

        // 判断方式2：定义一个包含所有大写字符的字符串，看当前字符时候在该字符串中
        // let judge = uppercaseLetter.includes(str[i])
        // 感觉第一种方式比较好
...
// 提取url 中的参数
const getRequestParams = urlStr => {
    const urlParams = urlStr.split('?')[1];
    const result = {};
    const list = urlParams.split('&');
    list.forEach(ele => {
